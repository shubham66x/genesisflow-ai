...
const Dashboard = () => {
  ...
  const handleSend = async () => {
    ...
    if (currentUser && currentUser.usage >= currentUser.maxUsage) {
      alert("You have reached your monthly limit of 100 free requests. Upgrade to continue.");
      return;
    }
    ...
    try {
      ...
      const response = await getAIResponse(updatedConvo?.messages || [], model);
      addMessage(convoId, "assistant", response, model);
      incrementUsage();
      setConversations(getConversations());
    } catch {
      addMessage(convoId, "assistant", "I apologize, but I encountered an error processing your request. Please try again.");
      setConversations(getConversations());
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="relative">
            ...
            >
              <span
                ...
                style={{ backgroundColor: MODEL_INFO[activeConvo?.model || selectedModel].color }}
              />
              <span>{MODEL_INFO[activeConvo?.model || selectedModel].name}</span>
                ...
                {(Object.keys(MODEL_INFO) as AIModel[]).map((model) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm text-left ${
                      (activeConvo?.model || selectedModel) === model ? "bg-secondary" : "hover:bg-secondary/50"
                    }`}
                  >
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center max-w-md">
                ...
                <p className="text-muted-foreground mb-6">
                  Choose your AI model above and start a conversation. Ask follow-up questions to go deeper. The AI remembers your full context.
                </p>
                ...
                <div className="flex justify-start">
                  <div className="glass gold-border rounded-xl px-4 py-3">
                    ...
                    <div className="flex gap-1">
                      <span className="w-2 h-2 bg-muted-foreground rounded-full opacity-40"></span>
                      <span className="w-2 h-2 bg-muted-foreground rounded-full opacity-60"></span>
                      <span className="w-2 h-2 bg-muted-foreground rounded-full opacity-80"></span>
                    </div>
                  </div>
                </div>
              )}
